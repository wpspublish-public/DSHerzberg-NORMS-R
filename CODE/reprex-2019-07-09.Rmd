...
```
If you run the above code, you'll see that the only remaining transformation  is to collapse the three `SS` = 83 rows into one, with a corresponding `raw` value of `3-5`. That means somehow recognizing that 3 and 5 are the lower and upper bounds of a range, because the solution will need to handle ranges of consecutive integers of any length.
