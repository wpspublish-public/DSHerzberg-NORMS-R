...
RVraw_by_agestrat <-
  suppressMessages(read_csv(here('INPUT FILES/RVraw_STAND.csv')
  )) %>% group_by(agestrat)

RV_tot_freq_agestrat <- RVraw_by_agestrat %>% count(RV_total) %>% 
  mutate(perc = round(100*(n/sum(n)), 4), cum_per = round(100*(cumsum(n)/sum(n)), 4), lag_tot = lag(RV_total), lag_cum_per = lag(cum_per))

RV_tot_desc_agestrat <-
  RVraw_by_agestrat %>% arrange(agestrat) %>% summarise(n = n(),
                                                         median = round(median(RV_total), 2),
                                                         mean = round(mean(RV_total), 2),
                                                         sd = round(sd(RV_total), 2)) %>%
  mutate(ES = round((mean - lag(mean))/((sd + lag(sd))/2),2), group = c(1:35))

agestrat <- RV_tot_desc_agestrat %>% pull(agestrat)
...
mean_plot <- ggplot(data = RV_tot_desc_agestrat, aes(group, mean)) +
  geom_point(
    col = "blue",
    fill = "blue",
    alpha = .5,
...
# ggplot(data = RVraw_by_agestrat, aes(RV_total)) +
#   geom_histogram(
#     binwidth = .2,
#     col = "red"
#   ) +
...
#   #   },
#   #   args = with(RVraw_by_agestrat, c(mean = mean(RV_total), sd = sd(RV_total), n
#   #                     = length(RV_total)))
#   # ) +
...
# ggplot(data = RV_tot_desc_agestrat, aes(group, mean)) +
#   geom_point(
#     col = "blue",
#     fill = "blue",
#     alpha = .5,
...
# 
# ggplot(data = RV_tot_desc_agestrat, aes(group, sd)) +
#   geom_point(
#     col = "red",
#     fill = "red",
#     alpha = .5,
...
RV_tot_age_lo1lo2_hi1hi2 <-
  full_join(
    RV_tot_freq_agestrat,
    (
      RV_tot_freq_agestrat %>%
        group_by(agestrat) %>%
...
norm_build1 <-
  RV_tot_freq_agestrat %>% left_join(RV_tot_age_lo1lo2_hi1hi2, by = 'agestrat') %>% mutate (
    IRS_lo1 = ((lo1 - lag_cum_per) / perc) * (RV_total - lag_tot) + lag_tot,
    IRS_lo2 = ((lo2 - lag_cum_per) /
                 perc) * (RV_total - lag_tot) + lag_tot,
...
rm(RV_tot_age_lo1lo2_hi1hi2, RVraw_by_agestrat, norm_build1)
