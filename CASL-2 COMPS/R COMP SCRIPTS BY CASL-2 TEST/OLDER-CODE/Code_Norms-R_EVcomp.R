# Generate norms for test scores with age-related developmental curve: CASL-2 EV
...
EVraw_by_agestrat <-
  suppressMessages(read_csv(here('INPUT FILES/EVraw_STAND.csv')
  )) %>% group_by(agestrat)

EV_tot_freq_agestrat <- EVraw_by_agestrat %>% count(EV_total) %>% 
  mutate(perc = round(100*(n/sum(n)), 4), cum_per = round(100*(cumsum(n)/sum(n)), 4), lag_tot = lag(EV_total), lag_cum_per = lag(cum_per))

EV_tot_desc_agestrat <-
  EVraw_by_agestrat %>% arrange(agestrat) %>% summarise(n = n(),
                                                         median = round(median(EV_total), 2),
                                                         mean = round(mean(EV_total), 2),
                                                         sd = round(sd(EV_total), 2)) %>%
  mutate(ES = round((mean - lag(mean))/((sd + lag(sd))/2),2), group = c(1:35))

agestrat <- EV_tot_desc_agestrat %>% pull(agestrat)
...
mean_plot <- ggplot(data = EV_tot_desc_agestrat, aes(group, mean)) +
  geom_point(
    col = "blue",
    fill = "blue",
    alpha = .5,
  ...
  scale_x_continuous(breaks = seq(1, 35, 1), labels = agestrat) +
...
# ggplot(data = EVraw_by_agestrat, aes(EV_total)) +
#   geom_histogram(
#     binwidth = .2,
#     col = "red"
#   ) +
...
#   #   },
#   #   args = with(EVraw_by_agestrat, c(mean = mean(EV_total), sd = sd(EV_total), n
#   #                     = length(EV_total)))
#   # ) +
...
# ggplot(data = EV_tot_desc_agestrat, aes(group, mean)) +
#   geom_point(
#     col = "blue",
#     fill = "blue",
#     alpha = .5,
...
# 
# ggplot(data = EV_tot_desc_agestrat, aes(group, sd)) +
#   geom_point(
#     col = "red",
#     fill = "red",
#     alpha = .5,
...
EV_tot_age_lo1lo2_hi1hi2 <-
  full_join(
    EV_tot_freq_agestrat,
    (
      EV_tot_freq_agestrat %>%
        group_by(agestrat) %>%
...
norm_build1 <-
  EV_tot_freq_agestrat %>% left_join(EV_tot_age_lo1lo2_hi1hi2, by = 'agestrat') %>% mutate (
    IRS_lo1 = ((lo1 - lag_cum_per) / perc) * (EV_total - lag_tot) + lag_tot,
    IRS_lo2 = ((lo2 - lag_cum_per) /
                 perc) * (EV_total - lag_tot) + lag_tot,
...
norm_build_med_hilo_sum <-
  ...
  ) %>% mutate(ES = (median - lag(median))/((hi_SD+lag(hi_SD)+lo_SD+lag(lo_SD))/4), group = 1:35) %>% select(group, everything())

# clean up environment
rm(EV_tot_age_lo1lo2_hi1hi2, EVraw_by_agestrat, norm_build1)
