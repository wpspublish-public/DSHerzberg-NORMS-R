...
SYN_tot_freq_agestrat <- SYNraw_by_agestrat %>% count(SYN_total) %>% 
  mutate(perc = round(100*(n/sum(n)), 4), cum_per = round(100*(cumsum(n)/sum(n)), 4), lag_tot = lag(SYN_total), lag_cum_per = lag(cum_per))

SYN_tot_desc_agestrat <-
  SYNraw_by_agestrat %>% arrange(agestrat) %>% summarise(n = n(),
                                                         median = round(median(SYN_total), 2),
                                                         mean = round(mean(SYN_total), 2),
                                                         sd = round(sd(SYN_total), 2)) %>%
  mutate(ES = round((mean - lag(mean))/((sd + lag(sd))/2),2), group = c(1:27))

agestrat <- SYN_tot_desc_agestrat %>% pull(agestrat)
...
SYN_tot_age_lo1lo2_hi1hi2 <-
  full_join(
    SYN_tot_freq_agestrat,
    (
      SYN_tot_freq_agestrat %>%
        group_by(agestrat) %>%
...
norm_build1 <-
  SYN_tot_freq_agestrat %>% left_join(SYN_tot_age_lo1lo2_hi1hi2, by = 'agestrat') %>% mutate (
    IRS_lo1 = ((lo1 - lag_cum_per) / perc) * (SYN_total - lag_tot) + lag_tot,
    IRS_lo2 = ((lo2 - lag_cum_per) /
                 perc) * (SYN_total - lag_tot) + lag_tot,
...
rm(SYN_tot_age_lo1lo2_hi1hi2, SYNraw_by_agestrat, norm_build1)
...
raw_to_SS_lookup_empty <- bind_cols(
  enframe(0:69, name = NULL, value = 'rawscore'), 
  data.frame(matrix(NA_real_, nrow = 70, ncol = 27)) %>% 
...
calc_cols <- c(paste0("calc_", 9:35))

# df containing empty calc columns that can be joined to other tables.
calc_cols_df <- bind_cols(
  enframe(0:71, name = NULL, value = 'rawscore'), 
  data.frame(matrix(NA_real_, nrow = 72, ncol = 35)) %>% 
...
tables_to_join <- list(raw_to_SS_lookup,
                       suppressMessages(read_csv(
                         here(
                           'CASL-2 COMPS/RAW-TO-SS-LOOKUP-TABLES-SPSS/SPSS-LOOKUPS/SYN-Age-Norms-Raw-to-SS-from-SPSS.csv'
                         )
                       )),
                       calc_cols_df
)
...
R_SPSS_comps_table <- tables_to_join %>% 
  ...
  select(
    rawscore, mo_60, a0500, calc_9, mo_63, a0530, calc_10, mo_66, a0560, calc_11, mo_69, a0590, 
...
write_csv(R_SPSS_comps_table, 
          here('CASL-2 COMPS/RAW-TO-SS-LOOKUP-TABLES-SPSS/COMPS/SYN-R-SPSS-SS-comps.csv'
          )
)
