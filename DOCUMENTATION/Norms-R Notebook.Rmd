...
* `tidyverse`: Many essential packages for data wrangling and analysis (`tidyr`, `dplyr`, `purrr`, `ggplot2`, `readr`, etc.)
...
#### Select method for entering input parameters; read input data
...
In the second block of code, we initialize an interim object `freq_csv` that allows us to drop two unneeded columns, using `select()` and the minus sign `-` to designate columns to remove. After the columns are dropped, the resulting table is written out, and then the interim object is removed from the global environment `base::rm()`.
```{r freq_desc, echo=34:54, eval=FALSE}
```
...
```{r RSmean_plot, echo=7:15, eval=FALSE}
```
...
#### Pause execution for user to examine descriptives table, plot

When the code is sourced, the following snippet pauses execution and prompts the user to examine the newly created plot of raw score means and SDs.
...
#### Pause execution for user input; generate histograms by agestrat

When the code is sourced, the following snippet pauses execution to ask whether the user wants to view histograms by agestrat of the score being normed.
...
```
###### COMMENTED CODE CHUNKS
The function that enables this interactive session is designated `hist_prompt`. `writeLines` prints instructions to the console, using the escape character `\n` to insert line breaks in the printed text.
```{r hist, echo=1:2, eval=FALSE}
```
The interactive session runs within the structure of a repeat loop `base::repeat{}`. In a repeat loop, the loop keeps iterating until R encounters an instruction to pass control outside the loop. Here, the default output of the iterating loop is the console prompt `Enter Y or N: `, which is printed as the `prompt` argument of `readline`, which pauses execution and awaits user input. That user input is coerced into a string `base::as.character`, and assigned to `show_hist` so it can be used to control logical flow in downstream code.

The `base::if` statement controls what happens if the user entry signifies "Yes" (i.e., the value of `show_hist` is among `%in%` this set of character values: `c('Y', 'y', 'Yes', 'yes')`. Under this condition, the code for generating the age-stratified histograms executes.
```{r hist, echo=3:6, eval=FALSE}
```
`ggplot` takes as input the raw data grouped by agestrat (named by `eval(as.name(paste0(score_name, '_raw_by_agestrat')))`), yielding histogram `geom_histogram`, which shows the distribution of the raw score being normed. Plot output is assigned to `hist_plot` so that the plot will print when the code is executed from source. `binwidth` controls the width of histogram columns, `col` controls the color of column outlines. You can adjust the values of these arguments to change the appearance of the graph. 
```{r hist, echo=7:11, eval=FALSE}
```
`scale_y_continuous` applies a new scale to y axis, using argument `breaks`. `seq` sets scale ranging from 0 to 20, in increments of 1. 
```{r hist, echo=12, eval=FALSE}
```
`labs` can be used to provide text labels for the graph, in this case titles and labels for the x- and y-axes.
```{r hist, echo=13, eval=FALSE}
```
`ggplot2::stat_function` superimposes an additional graphic layer on top of the histogram created by `ggplot`. Here `stat_function` draws normal probability curves (which are line graphs; the default `geom` of `stat_function` is a line). 

The argument `fun` defines an anonymous function with four arguments `function(x, mean, sd, n)`. In this function, the second, third, and fourth arguments are passed as arguments to another function `dnorm`. `dnorm` yields a normal probability density plot based on those input paramaters, which `function(x, mean, sd, n)` then multiplies by `n` to yield a drawn line that is proportional to the `n` of cases graphed in the histogram. 

`args` provides values for the four arguments (parameters) that are fed into `function(x, mean, sd, n)` and `dnorm`. These values are: the input data object, the `mean` of the score being normed, the `sd` of the score being normed, and the `n` of cases of the score being normed (with the latter being defined as the `length` of the column containing the score to be normed, which `base::length()` can process as a numerical vector).
```{r hist, echo=14:20, eval=FALSE}
```
`theme` changes non-data elements of the plot, in this case by removing minor gridlines. `element_blank` means 'draw nothing'.
```{r hist, echo=21, eval=FALSE}
```
`facet_wrap` generates a separate histogram for each `agestrat`. 
```{r hist, echo=22, eval=FALSE}
```
Explicit `print` call ensures that the plot will print when the code is executed from source.
```{r hist, echo=23, eval=FALSE}
```
...
The last line of this code chunk completes the specification of `full_join`, stating that the LHS and RHS tables are joined `by` the index variable `agestrat`. The data object at this point has the same number of rows as the full (non-summarized) input frequency table. `full_join` applies (recycles) the values of the two new variables, `min` and `lo1`, of which there is one value per agestrat, to all cells within a given agestrat in the larger (taller, more rows) table, thus filling out the table completely with those values.
```{r lo_hi_SD, echo=4:16, eval=FALSE}
```
Next we pipe the current data object (a tall table with the same number of rows as the input frequency table) through a series of transformations that allow it to become the basis for calculating `lo2`. The table is regrouped by `agestrat`, to provide a basis for later collapsing it back into a table with one row per agestrat. `mutate` then adds a new variable `flag` that is coded conditionally by `case_when`, such that cases are flagged if their value on `cum_per` is greater than `lo1` and less than 95. This coding on `flag` allows the data object to be filtered in a way that simplifies the calculation of `lo2` in a later procedure.  

The object is piped through `dplyr::filter`, which keeps only the flagged rows. `summarise` transforms the object again into a collapsed table with one row per agestrat, adding three variables: minimum value of `cum_per` per agestrat, maximum value of `cum_per` per agestrat, and first value of `lo1`. Because all values of `lo1` are identical within each agestrat, `first` is a convenient way of simply designating that value for `lo1` in the collapsed table.
```{r lo_hi_SD, echo=17:21, eval=FALSE}
```
The current data object is a table with one row for each `agestrat`. The next code chunk uses `mutate` to code `lo2` (possible values: 10, 15, 20, 25), `hi1` (possible values: 75, 80, 85, 90), and `hi2` (static value: 95). The conditional coding depends on varibles (`min`, `max`) that summarise the distribution of `cum_per` within each agestrat. Thus, R doesn't need to access the distributions in the original age-stratified frequency table to determine the correct values for `lo2` and `hi1`.
```{r lo_hi_SD, echo=22:39, eval=FALSE}
```
The next snippet cleans up the current data object for use in the next procedure. `select` drops the named columns preceded by `-` (minus sign). Without the minus sign, `select` would have kept only the named columns. Finally, `assign` gives the data object a name and assigns it to the global environment. The name has the suffix `_interim` because it will be immediately processed by the next snippet.
```{r lo_hi_SD, echo=40:41, eval=FALSE}
```
Next comes conditional code (`if-else`) to handle situations where the interim table is missing `agestrat` rows because of highly skewed raw-score distributions within age groups. These tables are identified when the logical condition `nrow(lo1lo2_hi1hi2_interim) != num_agestrat` returns `TRUE`. That is, the number of rows of the interim table is NOT equal to the number of age groups, which indicates missing rows.
```{r lo_hi_SD, echo=43, eval=FALSE}
```
To fix these problematic tables, we begin by converting the vector `agestrat` into a data frame with `tibble::enframe()`. The `agestrat` rows are contained in the column `value`, which we `select()` and `rename()` to yield a single-column data frame containing all values of `agestrat`.
```{r lo_hi_SD, echo=44:51, eval=FALSE}
```
This column is then joined to the interim table `lo1lo2_hi1hi2_interim`, creating a table with new row(s) for the formerly missing age group(s). Those new rows are `NA` for `lo1`, `lo2`, `hi1`, and `hi2`. `tidyr::fill()` is then used to fill in the missing values for these columns, using values from the lag row. `assign` names the data object for downstream code.
```{r lo_hi_SD, echo=52:62, eval=FALSE}
```
This next snippet specifies what happens in the `else` condition of the logic: when the interim object `lo1lo2_hi1hi2_interim` is NOT missing any `agestrat` rows. In this case, the interim object is simply assigned its downstream name without further processing.
```{r lo_hi_SD, echo=63:67, eval=FALSE}
```

<br>

#### Pause execution for user to examine normalization percentile points

When the code is sourced, the following snippet pauses execution and prompts the user to examine the table of `lo1`, `lo2`, `hi1`, and `hi2` percentile points, by `agstrat`, that will be used to normalize the raw score distribution.

###### VALID CODE TO RUN
```{r norm_perc_prompt, eval=FALSE}
...
```
###### COMMENTED CODE CHUNKS
`norm_perc_prompt()` is a function that takes no arguments and thus can be called with empty parentheses.
```{r norm_perc_prompt, echo=1, eval=FALSE}
```
`print()` displays the table of normalization percentile points on the console. `n = nrow()` is an argument that forces printing of the entire table, instead of just the first 10 rows.
```{r norm_perc_prompt, echo=2:3, eval=FALSE}
```
`writeLines()` takes a character vector and prints it to the console, where each vector element is printed on a new line. `strrep("\u2500", 80)` repeats a unicode horizontal box drawing character `u2400` 80 times, thus enclosing the printed text between horizontal lines.
```{r norm_perc_prompt, echo=4:11, eval=FALSE}
```
`readline` pauses execution and waits for the user to press [enter], at which point execution resumes.
```{r norm_perc_prompt, echo=14, eval=FALSE}
```
...
The process of normalizing each raw score distribution, by age group, continues. We add columns for the imputed median, and standard deviations above and below the median (lo_SD, hi_SD). Dividing the standard deviation into two components contributes to normalization by compensating for the skewness in the original raw score distributions.
...
###### COMMENTED CODE CHUNKS
The code initializes an output table `norm_build1`. The first input table is the age-stratified raw score frequency table `eval(as.name(paste0(score_name, '_freq_agestrat')))`. `dplyr::left_join()` matches this input table to the `lo1`, `lo2`, `hi1` and `hi2` normalizing cumulative percentage points for each `agestrat`, which are contained in a second input table `eval(as.name(paste0(score_name, '_age_lo1lo2_hi1hi2')))`. 
```{r med_SD_ES, echo=1:3, eval=FALSE}
```
The first set of new variables created by `dplyr::mutate()` are the imputed raw scores `IRS_` for the median `med`, `lo1`, `lo2`, `hi1` and `hi2`. The `IRS_` formula includes the raw score `eval(as.name(score_name)` associated with `lo1`, `lo2`, `hi1` or `hi2`, along with the percentage of cases `perc` in that same row. The formula also incorporates the raw score `lag_raw` and cumulative percentage `lag_cum_tot` from the _trailing_ row. `mutate()` calculates these values for every row, by `agestrat` even though eventually only the value in the row of `lo1` (for example) will flow through to the next set of calculations.
```{r med_SD_ES, echo=4:13, eval=FALSE}
```
Next `mutate()` creates the `dist_point` column. This column locates the rows in the table containing values of `cum_per` corresponding to the chosen values for `med`, `lo1`, `lo2`, `hi1` and `hi2`, and applies a label to flag these rows. In the flagged rows, the `lo1` label (for example) identifies the row where the value of `lo1` is less than or equal to the value of `cum_per` in that row, and greater than the value of `cum_per` in the trailing row. `case_when` is used to specify the logical conditions that identify the rows where the labels in `dist_point` are to be applied. The reason for flagging specific rows in this context is that we want to extract specific cells from columns (e.g., the `IRS_` columns) where a value is calculated for every row. We want to filter and pass only the rows corresponding to the `dist_point` labels to later operations.

Note the use of `TRUE ~ NA_character_` as the final argument of `case_when()`. `TRUE` specifies the _ELSE_ condition in the `case_when()` logic. When using `case_when` for conditional transformations, all RHS must evaluate to the same type of vector. In this instance, we want `TRUE` to evaluate to missing (`NA`), but this is a logical type, whereas the other RHS in this `case_when()` are all character. `NA_character_` provides missing values to a character vector.
```{r med_SD_ES, echo=14:25, eval=FALSE}
```
Now the filtering operation: `tidyr::drop_na()` drops any row that has `NA` in any column. In this instance, it drops all non-flagged rows in `dist_point`, resulting in a pared down table that includes, for each `agestrat`, only the five rows labeled `med`, `lo1`, `lo2`, `hi1` and `hi2`.
```{r med_SD_ES, echo=26, eval=FALSE}
```
The next snippet deals with normative samples where the score being normed shows a strong floor effect in the youngest age ranges, and/or a strong ceiling effect the oldest age ranges. In these situations, the preceeding code may not generate rows corresponding to the `lo1`, `lo2`, `hi1` and/or `hi2` distribution points. Downstream code requires the `norm_build1` object to have five rows (including `med`) per agestrat.

To handle these cases, the code first initializes an interim object `df_interim` and removes the `agestrat` grouping with `dplyr::ungroup()`. `mutate()` and `case_when` are then used to recode `dist_point`, replacing existing values with an alphabetical sequence that can be manipuluated downstream.
```{r med_SD_ES, echo=28:37, eval=FALSE}
```
`tidyr::complete()` is used to create new rows for levels of `agestrat` that have fewer than five rows in the interim data object. This is why `dist_point` was recoded previously; `complete()` needs to operate on a sequence of values (e.g., `A:E`) in order to recognize which values are missing from the sequence and create new rows corresponding to those missing values. `complete()` returns a table with five rows `A:E` per agestrat, generating new rows where needed. All new rows are `NA` for any columns not provided as arguments to `complete()`.
```{r med_SD_ES, echo=38:40, eval=FALSE}
```
The next step is to use `tidyr::fill()` to replace the `NA` in the new rows with values from lagging or trailing rows. The sequence `!!as.name(score_name):IRS_hi2` is passed to `fill()` to identify the columns to be filled in. (Note the use of the `!!` operator to unquote the variable name so that it can be evaluated as a name within a function). `fill()` can operate in either direction: the default is `"down"`, in which `fill()` works down the table, replacing `NA` in the target columns with values from the lagging row. To go up, we must use an explicit `.direction = "up"` argument, which causes `fill()` to work up the table, filling in values from the leading row. 

The end result a table in which the `NA` values for the imputed scores `IRS_lo1`, `IRS_lo2`, `IRS_med`, `IRS_hi1`, and `IRS_hi2` are replaced with values from adjacent rows, which is the only mechanism we have to "estimate" these missing values.
```{r med_SD_ES, echo=41:47, eval=FALSE}
```
The remaining lines of this snippet clean up the interim object for downstream processing. `dist_point` is recoded to restore meaningful labels for its levels. Grouping by `agestrat` is reapplied, and the now irrelavent object `norm_build1` is removed from the environment.
```{r med_SD_ES, echo=48:58, eval=FALSE}
```
