...
To specify `scale_y_break_options`, a numerical vector providing the numerical scale for the y-axis in plots generated by downstream code, we use `base::seq`. This function generates a numerical sequence, using arguments for starting number (`0`), ending number (`max_mean+(2*max_SD)`), and interval (`5`).

The ceiliing values, for the mean and SD along the y-axis of plots, are extracted from `scale_y_break_options`. This is done so that plots can be scaled appropriately for the differing possible score ranges of the raw score being normed. `base::which.min()` returns the sequence (position of a value in the vector `scale_y_break_options`)

***********START HERE WITH NEW WRITE OUT OF .CSVS FOR DESC AND FREQ*************
